...
export default defineEventHandler(async (event) => {
  ...
  let base = db.select().from(operarios).$dynamic()

  const conditions = []
  if (activoParam === 'true') conditions.push(eq(operarios.activo, true))
  else if (activoParam === 'false') conditions.push(eq(operarios.activo, false))
  if (q) conditions.push(like(operarios.nombre, `%${q}%`))
  if (conditions.length) base = base.where(conditions.length === 1 ? conditions[0] : and(...conditions))

  return base.orderBy(asc(operarios.nombre)).all()
})
